...
socket.on('messages', data => {
    const objDesnormalizado = normalizr.denormalize(data.result, msjsSchema, data.entities)
    let dataMsjs = objDesnormalizado.messages
    newId = searchLastId(dataMsjs)
    
    const html = dataMsjs.map(msj => {
        return `<div>
    ...
    const porcentaje = (100 - (JSON.stringify(data).length * 100 / JSON.stringify(dataMsjs).length))
    const compresionUi = `<h1>Compresión de mensajes en ${parseInt(porcentaje)}%</h1>`
